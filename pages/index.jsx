...
export default function Home() {
  ...
  const deriveMainShutdown = (payload) => (
    !!(payload?.shutdown ?? payload?.shutdownMain ?? payload?.shutdown_main ?? payload?.shutdownmain ?? false)
  );
  useEffect(() => {
    let alive = true;
    (async () => {
      ...
      for (const url of urls) {
        try {
          ...
          if (alive) { setShutdown(deriveMainShutdown(j)); }
          break;
        } catch {}
      }
    })();
    const onStorage = (e) => {
      if (e.key === 'site:flags') {
        try { const j = JSON.parse(e.newValue || '{}'); setShutdown(deriveMainShutdown(j)); } catch {}
      }
    };
    const onEvt = () => {
      try { const j = JSON.parse(localStorage.getItem('site:flags') || '{}'); setShutdown(deriveMainShutdown(j)); } catch {}
    };
