...
export async function requestOTP({ email, token }) {
  return postJson('/auth/request-otp', { email, token });
}
...
export async function submitNewPassword({ email, token, password, otp }) {
  return postJson('/auth/reset', { email, token, password, otp });
}
