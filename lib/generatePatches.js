...
function generatePatches(options = {}) {
  ...
  const cmd = 'git --no-pager log --name-status --pretty=format:%H|%at|%an|%ae|%s';
  let raw = '';
  try { raw = execSync(cmd, { cwd: repoRoot, stdio: ['ignore', 'pipe', 'ignore'] }).toString('utf8'); }
  catch (e) { throw new Error('Failed to read git log'); }
  ...
  if (newPatches.length) {
    const updated = { meta: { last_generated: new Date().toISOString() }, patches: [...existingPatches, ...newPatches] };
    fs.mkdirSync(path.dirname(dataPath), { recursive: true });
    fs.writeFileSync(dataPath, JSON.stringify(updated, null, 2));
    return updated;
  } else {
    const updated = { meta: { last_generated: new Date().toISOString() }, patches: existingPatches };
    fs.mkdirSync(path.dirname(dataPath), { recursive: true });
    fs.writeFileSync(dataPath, JSON.stringify(updated, null, 2));
    return updated;
  }
