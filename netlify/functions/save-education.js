...
async function saveToStorage(json, bucket = 'app_data', object = 'education.json') {
...
exports.handler = async function (event) {
  ...
  try {
    ...
    if (SUPABASE_URL && SUPABASE_SERVICE_ROLE_KEY) {
      ...
      let res = await fetch(url, { method: 'POST', headers: { apikey: SUPABASE_SERVICE_ROLE_KEY, Authorization: `Bearer ${SUPABASE_SERVICE_ROLE_KEY}`, 'Content-Type': 'application/json', Prefer: 'resolution=merge-duplicates' }, body: JSON.stringify({ id: 'education', data: body, updated_at: new Date().toISOString() }) });
      if (!res.ok) {
        const t = await res.text();
        if (res.status === 404 && t.includes('Could not find the table')) {
          await saveToStorage(body);
          await new Promise((r) => setTimeout(r, 150));
          res = await fetch(url, { method: 'POST', headers: { apikey: SUPABASE_SERVICE_ROLE_KEY, Authorization: `Bearer ${SUPABASE_SERVICE_ROLE_KEY}`, 'Content-Type': 'application/json', Prefer: 'resolution=merge-duplicates' }, body: JSON.stringify({ id: 'education', data: body, updated_at: new Date().toISOString() }) });
    ...
    return { statusCode: 500, body: `Save error: ${e.message || 'Failed to save education'}` };
  }
};
