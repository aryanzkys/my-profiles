...
exports.handler = async function () {
  try {
    if (SUPABASE_URL && SUPABASE_SERVICE_ROLE_KEY) {
      const url = `${SUPABASE_URL}/rest/v1/app_data?select=data&id=eq.education`;
      ...
      if (res.ok) {
        ...
        const sres = await fetch(`${SUPABASE_URL}/storage/v1/object/app_data/education.json`, { headers: { apikey: SUPABASE_SERVICE_ROLE_KEY, Authorization: `Bearer ${SUPABASE_SERVICE_ROLE_KEY}` } });
        if (sres.ok) {
    ...
    return { statusCode: 500, body: `Get error: ${e.message || 'Failed to get education'}` };
  }
};
